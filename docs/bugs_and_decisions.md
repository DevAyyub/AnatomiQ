...
| ATLAS-003 | AR Placement Modes | 🟡 Chunks 1–5 delivered | Chunks 1–2 device-verified; 3–5 await CORE-001 ratification + device pass |
...
### [EXECUTION — ATLAS-003] AR Placement Modes — chunks 1–4 (Space, Surface, Viewer, gestures)

**Decision 1 — three modes, one manipulation vocabulary (A.2: AR is a mode, not a wrapper).** `PlacementController : IPlacementProvider` (`Scripts/AR/`, `[DefaultExecutionOrder(-850)]`) owns the Surface/Space/Viewer state machine + placement policy; `BodyManipulator` applies pinch (scale everywhere) + two-finger drag (rotate in Viewer / reposition in AR) to CORE-002's `ModelRoot`. Single-finger input is left entirely untouched so CORE-004 can own organ selection (the D.1 gesture seam).
**Decision 2 — the anchor primitive stays in CORE-001.** The controller computes a target pose and calls `ARSessionManager.TryCreateContentAnchorAsync` / `RemoveContentAnchor`; it owns the plane + raycast managers (placement-specific) but never an `ARAnchorManager` ("all AR state changes through ARSessionManager only"). `ARSessionManager._planeManager` stays **None** — the controller's plane manager is the only one.
**Decision 3 — pose geometry is pure Core (`PlacementMath` / `ManipulationMath`), unit-tested AR-free.** The MonoBehaviours do only I/O + side effects.
**Decision 4 — mode reflects reality.** `CurrentMode` flips to Surface/Space only once the body actually anchors; the change-only `OnPlacementModeChanged` fires then. So `CurrentMode` is a sufficient "ready to manipulate" proxy — no separate placed-state.
**Decision 5 — surface seek is tap-free + auto-degrades.** A screen-centre raycast drives a reticle; the body auto-places once the centre ray holds on one plane for a short dwell. No stable surface within `_surfaceSeekTimeout` (5 s) → auto-fall back to Space (spec: surface >5s → offer Space). Space anchor failure → Viewer. Always a working mode.
*Status: chunks 1–2 (Space) device-verified; chunks 3–4 (Surface + Viewer framing + gestures) delivered as paste-in, compiled, not yet device-verified.*

---

### [EXECUTION — ATLAS-003] Chunk 5 — AppState reconciliation, D.4 session gating, mode-switcher + tracking-lost UI

**Decision 15 — AR session lifecycle + camera presentation centralize in CORE-001 (the rule).** D.4 session-gating and the Viewer↔AR camera swap go INTO `ARSessionManager` as `EnterAr()` / `ExitToViewer()` (+ `IsArSessionActive`). `PlacementController` decides WHEN (policy); `ARSessionManager` does HOW (mechanism), honoring "all AR state changes through ARSessionManager only." **This edits device-verified CORE-001 — awaits developer ratification + a device re-verify before commit.**
**Decision 16 — `_enterArOnStart = false` (D.4).** The app opens with the session DISABLED, `ARCameraBackground` off, `TrackedPoseDriver` off (camera doesn't chase device motion), camera cleared to `_viewerBackdropColor` — i.e. 3D Viewer, **no launch-time camera prompt**. `Start()` now only resolves availability (cheap, camera-free) so `Status` still distinguishes Unavailable from Initializing. **This changes CORE-001's prior device-verified "session up at Start" behavior;** the old path is one inspector toggle (`_enterArOnStart = true`) away for isolated CORE-001 testing. Authored `clearFlags`/`backgroundColor` are captured in `Awake` so AR mode restores them exactly.
**Decision 17 — the OS camera prompt fires on the AR-Mode tap, never at launch.** `RequestMode(Surface/Space)` → `EnterAr()` enables the session, and `EnterAr` is where `RequestCameraPermissionIfNeeded` runs. The UI shows the one-sentence rationale BEFORE calling `RequestMode`, so rationale precedes prompt.
**Decision 18 — placement is deferred, not blocked, during AR entry.** An AR request while `AppState != AR_ACTIVE` sets a pending mode + calls `EnterAr`; the body actually places when CORE-007 promotes `AppState` to `AR_ACTIVE` (in `HandleAppStateChanged`). A watchdog (`_arEntryTimeout`, 12 s) plus a `Status`-based early-out (PermissionDenied / Unavailable read off CORE-001) returns to Viewer if AR never comes up — the user is never stuck "entering AR."
**Decision 19 — AR_LIMITED = screen-pin, not just freeze.** On `AR_LIMITED` the body detaches from its anchor and is pinned a fixed distance in front of the camera each frame (`ComputeSpacePose`), so it stays usable through degraded tracking (CORE-001's `LockToScreenCenter` hint); on recovery (`AR_ACTIVE`) it re-anchors in Space. Pure freeze-at-last-anchor was the alternative.
**Decision 20 — arbitration is pure Core (`PlacementPolicy`).** `ResolveRequest(mode, state)` → `{ForceViewer, PlaceNow, EnterArThenPlace}`, plus `ShouldCollapseToViewer(state, hasPendingArEntry)` and `IsTrackingLocked(state)`. AR-package-free, fully EditMode-tested (`PlacementPolicyTests`, ~13 cases). The transient `AR_VIEWER_MODE` during bring-up is told apart from a genuine collapse by the pending-entry flag, never by AppState alone.
**Also (kept light):** the chunk-3 "5 s → auto Space" stayed as-is — the user-facing "offer Space" prompt was NOT built (optional/deferred). `EnterViewer` (deliberate) tears the session down via `ExitToViewer`; `FallBackToViewer` (a fluke anchor miss while AR is healthy) stays LIGHT — frames the body + flips mode but leaves the session up so a retry is instant. Two distinct intents, deliberately kept apart.

**UI (`AnatomiQ.UI`, mirrors the `PerformanceOverlay` house style — self-built canvas + TMP, reads only Core interfaces).** `PlacementModeSwitcher` — persistent bottom-centre segmented control (3D Viewer / Space / Surface), highlights the live mode, requests via `RequestMode`; shows the D.4 rationale before first AR entry; status line reads `IArTrackingProvider.Status` (a Core interface — no AR-pillar dependency) to pick "Tap to enable AR" vs "camera access off" vs "device doesn't support AR"; polls status at 4 Hz since `ArTrackingStatus` has no Core-side event and Initializing→PermissionDenied doesn't cross an AppState boundary. `TrackingLostToast` — non-modal `CanvasGroup` toast, fades in on `AR_LIMITED` / out otherwise (C.6), `blocksRaycasts=false` so it never eats touches.

**Localization (Build Environment C).** 11 keys owned as constants on the UI components; English values authored by `PlacementStringsInstaller` (Editor, `AnatomiQ ▸ Localization ▸ Add ATLAS-003 Placement Strings`, idempotent, mirrors `ThermalStringsInstaller`) — confirmed in console: "Added 11 new string(s) to 'UIStrings' (en)." **Assumption to confirm:** the runtime fetch is `LocalizationSettings.StringDatabase.GetLocalizedString("UIStrings", key)`, wrapped per-component in a `Localize(key)` helper that falls back to the key; if a project wrapper already exists, it's a one-method swap.

**Wiring needed before compile/run:** `AnatomiQ.UI.asmdef` → add `Unity.Localization` (already has TMP / UGUI / Input System); `AnatomiQ.AR.asmdef` → add `Unity.InputSystem` (for `BodyManipulator` touch + `TrackedPoseDriver` in `ARSessionManager`); on the XR Origin's `ARSessionManager` wire `_arCamera` / `_cameraBackground` / `_trackedPoseDriver`, leave `_planeManager` **None**, confirm `_enterArOnStart` off; both UI components dropped on **AppCore** (alongside `PerformanceOverlay`) with the `ServiceRegistry` assigned. Components and strings installed in-editor 2026-06-24.

**Files this chunk:** new — `Core/PlacementPolicy.cs`, `Tests/EditMode/PlacementPolicyTests.cs`, `UI/PlacementModeSwitcher.cs`, `UI/TrackingLostToast.cs`, `Editor/Localization/PlacementStringsInstaller.cs`; modified — `AR/ARSessionManager.cs` (CORE-001, gating + presentation), `AR/PlacementController.cs` (chunks 1–4 verbatim + chunk-5 reconciliation).

**Pending verification (gated on Decision 15/16 ratification):** EditMode suite green; XR-Sim + Poco — launches in Viewer w/ backdrop and no camera prompt → AR-Mode tap shows rationale → Continue prompts camera → Surface/Space/Viewer all place → cover lens forces AR_LIMITED (body screen-locks + toast) → uncover re-anchors. Chunk 6 (cleanup): undo CORE-002 `BodyRoot` +2 m nudge, delete `ARStatusLogger`, strip the `DEV —` ContextMenu triggers (PlacementController + BodyManipulator), add PlayMode tests over a mock anchor seam, final device gate.
...
| Remove temporary `ARStatusLogger` device probe | Used for CORE-001 device verification; delete the script + the AppCore component | Batched into ATLAS-003 chunk 6 cleanup (with the `BodyRoot` +2 m nudge undo + `DEV —` ContextMenu trigger removal) |
...
| CORE-001 session-gating change for D.4 (ATLAS-003 decisions 15–16) | 🟡 AWAITING RATIFICATION | `ARSessionManager` now opens session-disabled (`_enterArOnStart=false`) + gains `EnterAr`/`ExitToViewer`; changes the device-verified "session up at Start" path. Sign off + device re-verify before commit |
| UIStrings runtime accessor (ATLAS-003 UI) | 🟡 ASSUMPTION | Assumed `LocalizationSettings.StringDatabase.GetLocalizedString("UIStrings", key)` (wrapped in per-component `Localize`); confirm vs any existing project wrapper — one-method swap if different |

---

*Last updated: ATLAS-003 (AR Placement Modes) 🟡 chunks 1–5 delivered — chunk 5 this session. Engine + UI complete: `PlacementController` reconciles against CORE-007's `AppState` (sole writer untouched) — AR_VIEWER_MODE→force Viewer, AR_LIMITED→screen-pin body + tracking-lost toast (C.6), AR_ACTIVE→consume pending request / re-anchor; `RequestMode` arbitrates via the pure, EditMode-tested `PlacementPolicy`. D.4 honored: app opens in 3D Viewer with NO launch camera prompt — session gating + Viewer↔AR camera presentation moved into CORE-001 as `ARSessionManager.EnterAr()`/`ExitToViewer()` (decision 15), `_enterArOnStart=false` (decision 16), OS prompt on the AR-Mode tap (17), placement deferred behind AR bring-up with a 12 s watchdog (18). UI in `AnatomiQ.UI` (house style): `PlacementModeSwitcher` (segmented control + D.4 rationale + `IArTrackingProvider.Status` explainer) and `TrackingLostToast` (non-modal fade); 11 UIStrings keys authored via `PlacementStringsInstaller`. New: `PlacementPolicy`, `PlacementPolicyTests`, the two UI components, the installer; modified: `ARSessionManager` (CORE-001), `PlacementController`. **Open / gating:** developer ratification of the CORE-001 edit (decisions 15–16) + device re-verify before commit; the UIStrings runtime-accessor assumption; then the full D.4 + AR_LIMITED device pass. Chunk 6 = cleanup: undo CORE-002 `BodyRoot` +2 m nudge, delete `ARStatusLogger`, strip `DEV —` ContextMenu triggers, add PlayMode tests, device gate. Asmdef TODO: `Unity.Localization`→`AnatomiQ.UI`, `Unity.InputSystem`→`AnatomiQ.AR`.*
